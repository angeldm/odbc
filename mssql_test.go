...
var (
	mssrv    = flag.String("mssrv", "192.168.0.4", "ms sql server name")
	msdb     = flag.String("msdb", "Test", "ms sql server database name")
	msuser   = flag.String("msuser", "UCifecGest", "ms sql server user name")
	mspass   = flag.String("mspass", "Infor?*eurO", "ms sql server password")
...
var typeTests = []typeTest{
	...
	{"select cast('2007-08-05 12:35:29.123' as datetime)", match(time.Date(2007, 5, 8, 12, 35, 29, 123e6, time.Local))},
	{"select cast(NULL as datetime)", match(nil)},
	{"select cast('2007-08-05 12:35:29.123' as smalldatetime)", match(time.Date(2007, 5, 8, 12, 35, 0, 0, time.Local))},
